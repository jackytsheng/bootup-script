...
favourite = input('folder for favourite file : [Enter if current directory is used] ')
toDelete = input('folder for file to be deleted : [Enter if current directory is used] ') 
if favourite == "":
...
print("files be deleted {0} keeping {1} ".format(len(filesToBeDeleted),len(namesToDeleteSet) -len(filesToBeDeleted)))
